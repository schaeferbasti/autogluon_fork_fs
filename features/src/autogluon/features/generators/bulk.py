...
class BulkFeatureSelector(AbstractFeatureSelector):
    ...
    **kwargs :
    ...
    def _fit_transform(self, X: DataFrame, n_features: int, **kwargs) -> (DataFrame, dict):
        feature_metadata = self.feature_metadata_in
        for i in range(len(self.selectors)):
            ...
            for selector in self.selectors[i]:
                if selector.is_valid_metadata_in(feature_metadata):
                    ...
                    feature_df_list.append(selector.fit_transform(X, n_features=n_features, feature_metadata_in=feature_metadata, **kwargs))
