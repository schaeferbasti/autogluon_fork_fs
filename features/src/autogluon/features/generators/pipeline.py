...
class PipelineFeatureSelector(BulkFeatureSelector):
    """
    PipelineFeatureSelector is an implementation of BulkFeatureSelector with various smart defaults and edge case handling functionality to enable
    robust data handling.
    It is recommended that users base any custom feature selectors meant for end-to-end data transformation from PipelineFeatureSelector.
        Reference AutoMLPipelineFeatureSelector for an example of extending PipelineFeatureSelector.
    It is not recommended that PipelineFeatureSelector be used as a selector within any other selector's pre or post selectors.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pre_selectors is None:
            pre_selectors = [FillNaFeatureGenerator(inplace=True)]
        if post_selectors is None:
            post_selectors = [DropUniqueFeatureGenerator()]
    ...
    def fit_transform(self, X: DataFrame, y=None, model=None, n_max_features: int = None, feature_metadata_in: FeatureMetadata = None, **kwargs) -> DataFrame:
        X_out = super().fit_transform(X, y, model, n_max_features=n_max_features, feature_metadata_in=feature_metadata_in, **kwargs)
        self._compute_post_memory_usage(X_out)
        # TODO: Consider adding final check of validity/that features are reasonable.
        return X_out

    def _fit_transform(self, X: DataFrame, y=None, model=None, n_max_features: int = None, **kwargs):
        X_out, type_group_map_special = super()._fit_transform(X=X, y=y, model=model, n_max_features=n_max_features, **kwargs)
        X_out, type_group_map_special = self._fit_transform_custom(X_out=X_out, n_max_features=n_max_features, type_group_map_special=type_group_map_special, y=y)
        return X_out, type_group_map_special

    def _fit_transform_custom(self, X_out: DataFrame, n_max_features: int, type_group_map_special: dict, y=None) -> (DataFrame, dict):
        if len(list(X_out.columns)) == 0:
            self._is_dummy = True
            self._log(
                30,
            ...
            X_out = dummy_generator.fit_transform(X=X_out)
            type_group_map_special = copy.deepcopy(dummy_generator.feature_metadata.type_group_map_special)
            self.selectors = [[dummy_generator]]
            self._remove_features_in(features=self.features_in)
        return X_out, type_group_map_special
    ...
    def print_feature_metadata_info(self, log_level=20):
        ...
        if self._feature_metadata_in_unused.get_features():
            # TODO: Consider highlighting why a feature was unused
            #  (complex to implement, can check if was valid input to any selector in a selector group through feature chaining)
            self._log(
                log_level,
                f"\tUnused Original Features (Count: {len(self._feature_metadata_in_unused.get_features())}): "
                f"{self._feature_metadata_in_unused.get_features()}",
            ...
            self._log(
                log_level,
                "\t\tThese features were not used to generate any of the output features. "
                "Add a feature selector compatible with these features to utilize them.",
            )
            self._log(
                log_level,
                "\t\tFeatures can also be unused if they carry very little information, "
