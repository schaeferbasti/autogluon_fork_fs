...
from .pipeline import PipelineFeatureGenerator, PipelineFeatureSelector
...
class AutoMLPipelineFeatureSelector(PipelineFeatureSelector):
    """
    Pipeline feature selector with simplified arguments to handle most Tabular data including text and dates adequately.
    This is the default feature generation pipeline used by AutoGluon when unspecified.
    For more customization options, refer to :class:`PipelineFeatureSelector` and :class:`BulkFeatureSelector`.
    ...
    enable_numeric_features : bool, default True
        Whether to keep features of 'int' and 'float' raw types.
        These features are passed without alteration to the models.
        Appends IdentityFeatureGenerator(infer_features_in_args=dict(valid_raw_types=['int', 'float'])) to the selector group.
    enable_categorical_features : bool, default True
        Whether to keep features of 'object' and 'category' raw types.
        These features are processed into memory optimized 'category' features.
        Appends CategoryFeatureGenerator() to the selector group.
    enable_datetime_features : bool, default True
        Whether to keep features of 'datetime' raw type and 'object' features identified as 'datetime_as_object' features.
        These features will be converted to 'int' features representing milliseconds since epoch.
        Appends DatetimeFeatureGenerator() to the selector group.
    enable_text_special_features : bool, default True
        Whether to use 'object' features identified as 'text' features to generate 'text_special' features such as word count,
        capital letter ratio, and symbol counts.
        Appends TextSpecialFeatureGenerator() to the selector group.
    enable_text_ngram_features : bool, default True
        Whether to use 'object' features identified as 'text' features to generate 'text_ngram' features.
        Appends TextNgramFeatureGenerator(vectorizer=vectorizer, text_ngram_params) to the selector group. See text_ngram.py for valid parameters.
    enable_raw_text_features : bool, default False
        Whether to use the raw text features. The generated raw text features will end up with '_raw_text' suffix.
        For example, 'sentence' --> 'sentence_raw_text'
    enable_vision_features : bool, default True
    ...
    **kwargs :
    ...
    --------
    >>> from autogluon.tabular import TabularDataset
    >>> from autogluon.features.selectors import AutoMLPipelineFeatureSelector
    >>>
    >>> feature_selector = AutoMLPipelineFeatureSelector()
    >>>
    >>> label = 'class'
    >>> train_data = TabularDataset('https://autogluon.s3.amazonaws.com/datasets/Inc/train.csv')
    >>> X_train = train_data.drop(labels=[label], axis=1)
    ...
    def __init__(
        ...
        if "selectors" in kwargs:
            raise KeyError(
                f"selectors is not a valid parameter to {self.__class__.__name__}. "
                f"Use {PipelineFeatureSelector.__name__} to specify custom selectors."
            )
        if "enable_raw_features" in kwargs:
            enable_numeric_features = kwargs.pop("enable_raw_features")
            logger.warning(
        ...
        selectors = self._get_default_selectors(vectorizer=vectorizer)
        super().__init__(selectors=selectors, **kwargs)

    def _get_default_selectors(self, vectorizer=None):
        selector_group = []
        if self.enable_numeric_features:
        ...
        if self.enable_categorical_features:
            selector_group.append(self._get_category_feature_generator())
        if self.enable_datetime_features:
            selector_group.append(DatetimeFeatureGenerator())
        if self.enable_text_special_features:
            selector_group.append(TextSpecialFeatureGenerator())
        if self.enable_text_ngram_features:
            selector_group.append(TextNgramFeatureGenerator(vectorizer=vectorizer, **self.text_ngram_params))
        if self.enable_vision_features:
            selector_group.append(
                IdentityFeatureGenerator(
                    infer_features_in_args=dict(
...
class AutoMLInterpretablePipelineFeatureSelector(AutoMLPipelineFeatureSelector):
    def _get_category_feature_generator(self):
        return CategoryFeatureGenerator(
            minimize_memory=False, maximum_num_cat=10, post_selectors=[OneHotEncoderFeatureGenerator()]
        )
